...
def test_arcface_basic_forward():
    ...
    arcface = ArcFace(in_features=embedding_dim, out_features=num_classes, s=64.0, m=0.50)
...
def test_arcface_margin_application():
    ...
    for i, label in enumerate(labels):
        ...
        assert abs(target_output - s * target_cosine) > 1e-3, \
        ...
        for j in range(num_classes):
            if j != label:
                ...
                assert abs(non_target_output - s * non_target_cosine) < 1e-5, \
...
def test_arcface_scale_factor():
    ...
    for s in [32.0, 64.0, 128.0]:
        arcface = ArcFace(in_features=embedding_dim, out_features=num_classes, s=s, m=0.50)
        embeddings = F.normalize(torch.randn(batch_size, embedding_dim), p=2, dim=1)
        labels = torch.randint(0, num_classes, (batch_size,))
...
def test_arcface_gradient_flow():
    ...
    assert not torch.allclose(arcface.weight.grad, torch.zeros_like(arcface.weight.grad)), \
...
def test_arcface_numerical_stability():
    ...
    assert not torch.isnan(output_small).any(), "Output should handle small embeddings without NaN"
    ...
    assert not torch.isnan(output_large).any(), "Output should handle large embeddings without NaN"
...
def test_arcface_different_margins():
    ...
    for m in [0.1, 0.3, 0.5, 0.7]:
        arcface = ArcFace(in_features=embedding_dim, out_features=num_classes, s=64.0, m=m)
        embeddings = F.normalize(torch.randn(batch_size, embedding_dim), p=2, dim=1)
        labels = torch.randint(0, num_classes, (batch_size,))
