...
def generate_2d_gaussian_points(N: int, mean: np.ndarray, cov: np.ndarray, random_seed: int = None) -> np.ndarray:
    """
    ...
    if cov.ndim == 0:
        ...
        raise ValueError(f"cov must be a scalar, 1D array, or 2D array, got {cov.ndim}D array")
