...
def torch_load_with_prefix_removal(model_param_path):
    ...
    if count_of_removed_prefix > 0:
        print(
            f"Removed {count_of_removed_prefix} prefix 'module.' from model parameters"
        )
    if count_of_removed_prefix != len(list(model_param.keys())):
        print(
            f"Number of removed prefix 'module.' {count_of_removed_prefix} does not match the number of model parameters {len(list(model_param.keys()))}"
        )
...
def run_in_background(target_task, log_file, args=(), kwargs={}):
    ...
    def task_wrapper():
        ...
        """
        print(
            f"Background process started. Logging to '{log_file}'. PID: {multiprocessing.current_process().pid}"
        )
...
class MainCmdManager:
    ...
    def main(self):
        ...
        parser.add_argument(
            "--cmd", "-c", type=str, required=True, help="Command to execute"
        )
        ...
        if args.cmd not in self.commands:
            parser.error(
                f"Unknown command: {args.cmd}. Available commands: {list(self.commands.keys())}"
            )
