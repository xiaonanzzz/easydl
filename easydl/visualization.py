...
class PairwiseScoreAnalysisPlot:
    ...
    def analyze(self, n_bins: int = 50):
        ...
        precision_list_full, recall_list_full, threshold_list = precision_recall_curve(
            self.pairwise_gt_list, 
            self.pairwise_score_list
        )
        ...
        for i in range(len(self.distribution_bins) - 1):
            ...
            mask = (self.pairwise_score_list >= bin_left) & (self.pairwise_score_list < bin_right)
    ...
    def plot(self, figsize=(12, 6)):
        ...
        ax1.bar(self.bin_centers, self.bin_histograms[0], width=bin_width * 0.8, 
                alpha=0.6, label='Negative pairs (GT=0)', color='blue', align='center')
        
        # Plot normalized density histogram for positive values (gt=1)
        ax1.bar(self.bin_centers, self.bin_histograms[1], width=bin_width * 0.8, 
                alpha=0.6, label='Positive pairs (GT=1)', color='orange', align='center')
        ...
        ax2.plot(self.subsampled_thresholds, self.subsampled_precision, 
                color='green', label='Precision', linewidth=2, alpha=0.8, marker='o', markersize=3)
        ax2.plot(self.subsampled_thresholds, self.subsampled_recall, 
                color='red', label='Recall', linewidth=2, alpha=0.8, linestyle='--', marker='s', markersize=3)
        ...
        title = (f"Pairwise Score Distribution and Precision/Recall\n"
