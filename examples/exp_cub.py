...
from easydl.dml.evaluation import (
    DeepMetricLearningImageEvaluatorOnEachEpoch,
...
from easydl.dml.trainer import (
    DeepMetricLearningImageTrainverV871,
...
from easydl.public_dataset.cub import (
    ...
    get_test_dataset_with_image_and_encoded_labels,
    get_train_dataset_with_image_and_encoded_labels,
)
from easydl.utils import AcceleratorSetting, MainCmdManager, WorkingDirManager
...
def do_dml_experiment_with_cub_dataset():
    ...
    DeepMetricLearningImageTrainverV971(
        ds_train,
        ds_train.get_number_of_classes(),
        model_name=args.model_name,
        loss_name=args.loss_name,
        embedding_dim=ExpCubConfig.embedding_dim,
        batch_size=ExpCubConfig.batch_size,
        num_epochs=ExpCubConfig.num_epochs,
        lr=ExpCubConfig.lr,
    )
    ...
    evaluator = DeepMetricLearningImageEvaluatorOnEachEpoch(
        ds_test,
        args.model_name,
        ExpCubConfig.embedding_dim,
        args.exp_dir,
        ExpCubConfig.num_epochs,
        args.exp_dir,
    )
...
def exp_eval_a_model_on_cub_test_set(embedding_model, image_to_tensor_transform):
    ...
    metrics = StandardEmbeddingEvaluationV1.evaluate_given_dataset(
        ds_test, embedding_model
    )
    return metrics
...
def exp_eval_pretrained_resenet18():
    ...
    metrics = exp_eval_a_model_on_cub_test_set(
        tensor_to_embedding_model, image_to_tensor_transform
    )
    print(metrics)
...
def exp_run_evaluation_on_each_epoch():
    ...
    DeepMetricLearningImageEvaluatorOnEachEpoch(
        ...
        "tmp/exp_cub_v971",
        ExpCubConfig.num_epochs,
        "tmp/exp_cub_v971_evaluation_report",
    )


def main():
    main_cmd_manager = MainCmdManager()
    main_cmd_manager.register_main_cmd(
        "do_dml_experiment_with_cub_dataset", do_dml_experiment_with_cub_dataset
    )
    main_cmd_manager.register_main_cmd(
        "exp_eval_pretrained_resenet18", exp_eval_pretrained_resenet18
    )
    main_cmd_manager.register_main_cmd(
        "exp_run_evaluation_on_each_epoch", exp_run_evaluation_on_each_epoch
    )
    main_cmd_manager.register_main_cmd(
        "do_dml_experiment_with_cub_dataset_with_data_augmentation",
        do_dml_experiment_with_cub_dataset_with_data_augmentation,
    )
